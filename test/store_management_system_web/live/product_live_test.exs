defmodule StoreManagementSystemWeb.ProductLiveTest do
  ...
  @create_attrs %{current_inventory_count: 42, name: "some name", retail_price: 120.5, vendor: "some vendor", wholesale_price: 120.5}
  ...
  describe "Index" do
    ...
    test "lists all product", %{conn: conn, product: product} do
      {:ok, _index_live, html} = live(conn, ~p"/product")
    ...
    test "saves new product", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, ~p"/product")

      assert index_live |> element("a", "New Product") |> render_click() =~
               "New Product"

      assert_patch(index_live, ~p"/product/new")
      ...
      assert_patch(index_live, ~p"/product")
    ...
    test "updates product in listing", %{conn: conn, product: product} do
      ...
      assert_patch(index_live, ~p"/product/#{product}/edit")
  ...
  describe "Show" do
    ...
    test "displays product", %{conn: conn, product: product} do
      {:ok, _show_live, html} = live(conn, ~p"/product/#{product}")
    ...
    test "updates product within modal", %{conn: conn, product: product} do
      {:ok, show_live, _html} = live(conn, ~p"/product/#{product}")

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit Product"

      assert_patch(show_live, ~p"/product/#{product}/show/edit")
      ...
      assert_patch(show_live, ~p"/product/#{product}")
